...
class Config
{
     ...
     * @return mixed
     */
    public static function iniGet($key , $range = 'database' , $default = null){
        $value = self::get($key);
        if(!APP_DEPLOY && !empty($value)){
            return $value;
        }else{
            if (class_exists("Yaconf"))
                return \Yaconf::get($range.".".$key , $default);
            else
                throw new ClassNotFoundException("Yaconf Class is not found.");
        }
    }
}
