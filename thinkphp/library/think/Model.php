...
class Model {
    ...
    // 查询表达式参数
    protected $options          =   array('where' => [] , 'strict' => true);
    protected $_validate        =   array();  // 自动验证定义
    protected $_auto            =   array();  // 自动完成定义
    protected $_map             =   array();  // 字段映射定义
    protected $_scope           =   array();  // 命名范围定义p
    // 是否自动检测数据表字段信息
    ...
    public function __construct($name='',$tablePrefix='',$connection='') {
        ...
        if(!empty($connection)){
            ...
            $this->dbName = $this->connection['database'];
        }
        ...
        }elseif(isset($this->connection['prefix'])){
            $this->tablePrefix = $this->connection['prefix'];
    ...
    protected function _checkTableInfo() {
        ...
        if(empty($this->fields)) {
            // 如果数据表字段没有定义则自动获取
            if(Config::get('database.db_fields_cache')) {
                $db   =  $this->dbName?:Config::get('database.database');
                $fields = $this->F('_fields/'.strtolower($db.'.'.$this->tablePrefix.$this->name));
                if($fields) {
                    $this->fields   =   $fields;
                    if(!empty($fields['_pk'])){
                        $this->pk       =   $fields['_pk'];
    ...
    public function flush() {
        ...
        if(Config::get('database.db_fields_cache')){
            // 永久缓存数据表信息
            $db   =  $this->dbName?:$this->connection['database'];
            $tableName = empty($this->tableName) ? $this->name : $this->tableName;
            $this->F('_fields/'.strtolower($db.'.'.$this->tablePrefix.$tableName),$this->fields);
        }
    }
     ...
     * @return mixed
     */
    public function add($data='',$options=array(),$replace=false , $returnBind = false) {
        if(empty($data)) {
            // 没有传递数据，获取当前数据对象的值
            if(!empty($this->data)) {
                $data           =   $this->data;
        ...
        // 写入数据到数据库
        $result = $this->db->insert($data,$options,$replace,$returnBind);
        if($returnBind){
            return $result;
        }
     ...
     * @return bool
     */
    public function getTransSaveBind($data = []){
     ...
     * @return mixed
     */
    public function getTransAddBind($data=[]){
     ...
     * @return boolean
     */
    public function save($data='',$options=array(),$returnBind = false) {
        if(empty($data)) {
            // 没有传递数据，获取当前数据对象的值
            if(!empty($this->data)) {
                $data           =   $this->data;
        ...
        $result     =   $this->db->update($data,$options,$returnBind);
        if($returnBind){
            return $result;
        }
        if(false !== $result && is_numeric($result)) {
    ...
    public function delete($options=array()) {
        // 软删除
        if($this->softDelete && !empty(Config('database.soft_delete_field'))){
            return $this->save([Config('database.soft_delete_field') => NOW_TIME] , $options);
        }
    ...
    protected function _parseOptions($options=array()) {
        ...
        // 软删除
        if($this->softDelete && !empty(Config('database.soft_delete_field'))){
            if(!empty($options['where']['_string'])){
                $options['where']['_string'] .= " AND ".Config('database.soft_delete_field')." = 0";
            }else{
                $options['where'][Config('database.soft_delete_field')] = 0;
            }
        }
    ...
    protected function _read_data($data) {
        // 检查字段映射
        if(!empty($this->_map) && Config::get('database.read_data_map')) {
     ...
     */
     public function create($data='',$type='') {
        ...
        if(isset($fields)) {
            ...
            // 判断令牌验证字段
            if(Config::get('database.token_on'))   $fields[] = Config::get('database.token_name', null, '__hash__');
            foreach ($data as $key=>$val){
    ...
    public function autoCheckToken($data) {
        ...
        if(Config::get('database.token_on')){
            $name   = Config::get('database.token_name', null, '__hash__');
            ...
            // 开启TOKEN重置
            if(Config::get('database.token_reset')) unset($_SESSION[$name][$key]);
            return false;
        }
        return true;
    }
    ...
    public function check($value,$rule,$type='regex'){
        ...
        switch($type) {
            ...
            case 'ip_allow': // IP 操作许可验证
                return in_array(request()->ip(),explode(',',$rule));
     ...
     * @return mixed
     */
    public function query($sql,$parse=false,$returnType = 1) {
        ...
        if(empty($this->options['bind'])){
            return $this->db->query($sql, false , $returnType);
        }else{
            return $this->db->queryBind($sql , $this->options['bind'] , $returnType);
        }
    }
     ...
     * @return mixed
     */
    public function query_row($sql,$parse=false) {
        if(!is_bool($parse) && !is_array($parse)) {
            $parse = func_get_args();
            array_shift($parse);
        }
     ...
     * @return false | integer
     */
    public function executeBind($sql,$bind) {
        return $this->db->executeBind($sql , $bind);
    }
     ...
     * @return array
     */
    public function getDbFieldsType($field = ''){
        if(isset($this->options['table'])) {// 动态指定表名
            if(is_array($this->options['table'])){
                $table  =   key($this->options['table']);
            }else{
        ...
        if($this->fields) {
            ...
            return $this->fields['_type'][$field];
        }
        return false;
    }
    ...
    public function table($table) {
        ...
        if(is_array($table)) {
            ...
            if(!empty($this->tablePrefix) && strpos($table , $this->tablePrefix) !== 0){
