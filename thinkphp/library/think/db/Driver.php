...
abstract class Driver {
     ...
     * @return mixed
     */
    public function queryBind($sql , $bind = [] , $returnType){
        $this->parseBind(!empty($bind)?$bind:array());
        $result = $this->query($sql , false, $returnType);
        return $result;
    }
     ...
     * @return mixed
     */
    public function query($str,$fetchSql=false , $returnType = 1) {
        ...
        try{
            ...
            $sqlErrorHandlerClassName = config('sql_error_handler');
            if(!is_null($sqlErrorHandlerClassName) && class_exists($sqlErrorHandlerClassName)){
                $sqlErrorHandler = new $sqlErrorHandlerClassName();
                if(method_exists($sqlErrorHandler , 'render')){
                    $errorInfo = [
                        'sql'     => $str,
                    ...
                    $sqlErrorHandler->render($errorInfo);
                    config('SaveErrorLogStatus' , 1);
                }
            }
        ...
        if ( false === $result ) {
            ...
            return $this->getResult($returnType);
        }
    }
     ...
     * @return mixed
     */
    public function executeBind($str , $bind){
        $this->bind = $bind;
    ...
    public function execute($str,$fetchSql=false) {
        ...
        try{
            ...
            $sqlErrorHandlerClassName = config('sql_error_handler');
            if(!is_null($sqlErrorHandlerClassName) && class_exists($sqlErrorHandlerClassName , false)){
                $sqlErrorHandler = new $sqlErrorHandlerClassName();
                if(method_exists($sqlErrorHandler , 'render')){
                    $errorInfo = [
                        'sql'     => $str,
     ...
     * @return array
     */
    private function getResult($returnType) {
        //返回数据集
        if($returnType == 1){
     ...
     */
    public function insert($data,$options=array(),$replace=false,$returnBind = false) {
        $values  =  $fields    = array();
     ...
     * @return false | integer | array
     */
    public function update($data,$options,$returnBind = false) {
        $this->model  =   $options['model'];
        $this->parseBind(!empty($options['bind'])?$options['bind']:array());
    ...
    protected function debug($start) {
        if($this->config['debug']) {// 开启数据库调试模式
            if($start) {
                $this->G('queryStartTime');
            }else{
                $this->modelSql[$this->model]   =  $this->queryStr;
                //$this->model  =   '_think_';
                // 记录操作结束时间
                $this->G('queryEndTime');
                Log::record($this->queryStr.' [ RunTime:'.$this->G('queryStartTime','queryEndTime').'s ]','SQL');
            }
        }
    }
     ...
     * @return mixed
     */
    protected function N($key, $step=0,$save=false) {
        static $_num    = array();
        if (!isset($_num[$key])) {
     ...
     * @return mixed
     */
    protected function G($start,$end='',$dec=4) {
        static $_info       =   array();
        static $_mem        =   array();
